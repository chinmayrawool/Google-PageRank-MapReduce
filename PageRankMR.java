   ...
   public int run( String[] args) throws  Exception {
      ...
      long N = job.getCounters().findCounter("Output", "Output").getValue();
       ...
       //Job2 has the 1st output taking Num of nodes as an input 
		...
		//Set the Mapper and Reducer class for Job 2
        job2.setMapperClass(LinkGraphMapper.class);
        job2.setReducerClass(LinkGraphReducer.class);
		//Set the Map Output Key and Value Class for Job 2
        ...
        Job job3;
        //Set the iterations to 10 to converge the page rank
        for (int i = 1; i <= iterations; i++) {
            ...
            //Set the Mapper and Reducer class for Job 3
            job3.setMapperClass(PageRankMapper.class);
            job3.setReducerClass(PageRankReducer.class);
			
			job3.setOutputFormatClass(TextOutputFormat.class);
			//Set the Map Output Key and Value Class for Job 3
            job3.setMapOutputKeyClass(Text.class);
            job3.setMapOutputValueClass(Text.class);
            job3.waitForCompletion(true);
        }
		
        //To sort in Descending order and the input is the output of the last iteration i.e. iter10
        Job job4 = Job.getInstance(getConf(), "Sorting");
        job4.setJarByClass(this.getClass());
		//Input is the output of last iteration i.e.iter10
        FileInputFormat.addInputPaths(job4, args[0] + "iter10");
		//Output path is given as argument
        FileOutputFormat.setOutputPath(job4, new Path(args[1]));
		//Number of reducer set to 1 for global sorting.
        job4.setNumReduceTasks(1);
        job4.getConfiguration().set("InputFiles", "" + N);
		//Set the Mapper and Reducer class for Job 4
        job4.setMapperClass(SortingMapper.class);
        job4.setReducerClass(SortingReducer.class);
		//Set the Map Output Key and Value Class for Job 4
...
public static class XmlPatternMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        page = page.trim();
        String title = null;
        //Match the pattern and group it together
        Matcher titleMatcher = titlePattern.matcher(page);
        if (titleMatcher.find()) {
            //Title match found. Group the titles together
...
//nodeSeparator, linkDelim, Outlinks and iteration declaration
int iterations = 10;
